...
template <>
inline
void doTransform(const geometry_msgs::PolygonStamped& t_in, geometry_msgs::PolygonStamped& t_out, const geometry_msgs::TransformStamped& transform)
{
  ...
  for(unsigned int i = 0; i < t_in.polygon.points.size(); i++) {
    KDL::Vector v_out = gmTransformToKDL(transform) * KDL::Vector(t_in.polygon.points[i].x, t_in.polygon.points[i].y, t_in.polygon.points[i].z);
    p_out.x = v_out[0];
    p_out.y = v_out[1];
    p_out.z = v_out[2];
